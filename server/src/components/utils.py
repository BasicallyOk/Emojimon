...
class Utils:
    ...
    def json_encoder(self, obj):
        ...
        data = self.__dict_traversal(data)
        with open(f"{obj.get_id()}.json", mode="w+", encoding="UTF-8") as f:
            json.dump(data, f)

    def __dict_traversal(self, data: dict):
        """
        ...
        for key in data.keys():
            ...
            if hasattr(val, '__dict__'):
                data[key] = self.__dict_traversal(val.__dict__)
                ...
                data[key] = self.__dict_traversal(val)
    ...
    def __arbitrary_list_depth_traversal(self, lst: list):
        ...
        for i in range(len(lst)):
            if isinstance(lst[i], list):
                ...
                lst[i] = self.__dict_traversal(lst[i].__dict__)
        return lst
